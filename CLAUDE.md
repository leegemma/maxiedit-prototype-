...
- Native wrappers: [Capacitor 6.x](https://capacitorjs.com/) ([package.json](package.json), [capacitor.config.json](capacitor.config.json), `www/`). [android/](android/) is generated by `npm run cap:add:android` and committed; [ios/](ios/) is generated by `npm run cap:add:ios` and committed. Build outputs (`build/`, `Pods/`, `xcuserdata/` …) are git-ignored.
...
- **Clean clone** (native build, no Korean/spaces in path): `~/dev/maxiedit-prototype-`

Android Studio, Xcode, and CocoaPods all refuse to operate inside the iCloud + Korean path, so all native build commands (`npm install`, `npm run cap:add:android` / `cap:add:ios`, `cap:open:*`, `cap:build:*`) MUST be run from `~/dev/maxiedit-prototype-`. Web edits/commits can happen in either; pull on the other side to stay in sync.
...
- `ios/App/App/Info.plist` carries `NSPhotoLibraryUsageDescription` ("선택한 사진/영상을 편집하기 위해 사진 라이브러리에 접근합니다.") and `NSPhotoLibraryAddUsageDescription` ("편집한 결과물을 사진 앱에 저장합니다."). Without these, iOS terminates the app on first photo access.
- The plugin (last published 2022) doesn't compile on Xcode 26 / iOS 17+ out of the box. Four upstream fixes live in [patches/cordova-plugin-photo-library+2.3.1.patch](patches/cordova-plugin-photo-library+2.3.1.patch) and are re-applied automatically on every `npm install` via the `postinstall` hook ([patch-package](https://www.npmjs.com/package/patch-package)). Patched: `UIImageOrientation` → `UIImage.Orientation` (Swift 4.2+), `String.characters.count` → `String.count` (Swift 4.0+), unsafe `info!["PHImageFileURLKey"] as! URL` → safe optional binding (iOS 13 deprecation), and `as! Float` of an NSNumber option → `(NSNumber).floatValue` (Swift NSNumber/Float bridge quirk). Don't edit `node_modules/cordova-plugin-photo-library/` by hand — re-run `npx patch-package cordova-plugin-photo-library` after any change so the patch file stays the source of truth.
- The plugin's `cdvphotolibrary://` URL scheme works under UIWebView's URLProtocol but is NOT intercepted by Capacitor's WKWebView (URLSchemeHandler). So `loadDeviceRecentPhotos()` calls `getThumbnail(item)` per asset to receive a real `Blob`, then wraps it with `URL.createObjectURL()` for the picker grid — see the comment in [index.html](index.html).
...
```bash
npm install                  # one-time (also installs @capacitor/ios)
npm run cap:add:ios          # generates ios/ on first run (runs `pod install`)
npm run cap:open:ios         # opens App.xcworkspace in Xcode
# or
npm run cap:build:ios        # CLI simulator build (Debug, iphonesimulator)
```

`npm run cap:sync:ios` after every `index.html` change to push the new web bundle into `ios/App/App/public/` (parallel to `cap:sync` for Android).

**Always open `App.xcworkspace`, never `App.xcodeproj`** — the workspace is what wires CocoaPods in (Capacitor pulls plugins through Pods). Opening the bare project gives "module not found" errors.

Capacitor iOS config (`capacitor.config.json` → `ios`): `contentInset: always` (status-bar safe area), `backgroundColor: #000000` (no white flash on launch — same intent as Android), `scrollEnabled: false` (full-screen lock; the app frame handles its own scroll regions).

Prereqs: Mac, Xcode 16+, CocoaPods (`brew install cocoapods`), Apple Developer Program membership for device deployment / TestFlight.
