...
| 날짜 | 커밋 | 요약 |
|---|---|---|
| 2026-05-05 | (this commit) | 홈 carousel 복구 — "ig 피드 만들기" 섹션을 트랙 밖 unconditional로 되돌려 모든 탭에서 항상 노출 (이전 커밋 f6fb9be에서 `data-tab="upcoming"` 안으로 이동했었음). 준비중 탭 슬롯에는 `.home-section--empty` placeholder ("준비 중인 콘텐츠가 없어요") 추가해 carousel 수학(`width: calc(200% + 16px)` + 양쪽 50%) 무결성 유지. `.home-tab-track + .home-section { margin-top: 24px }`로 트랙과 ig 섹션 사이 간격 확보 |
